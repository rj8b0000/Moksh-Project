  ...
  const onVerifyEmail = async () => {
    ...
    try {
      ...
      await sendEmailVerification(auth.currentUser)
        .then(async() => {
          await AsyncStorage.setItem('USER_ID',JSON.stringify(auth?.currentUser?.uid));
  ...
  return (
    <LinearGradient colors={['#f0ba2c', '#f9af0c', '#ffa901']} style={styles.linearGradient}>
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <Text style={{ color: '#000', fontSize: 22, fontFamily: 'Inter-Medium' }}>We would send you logic link on entered E-mail</Text>
        <View style={{ margin: 10, marginVertical: 40 }}>
        ...
        <View style={{ marginTop: '7%', width: '90%', alignSelf: 'center' }}>
          <Pressable style={{ backgroundColor: '#fefffe', justifyContent: 'center', alignItems: 'center', height: 45, width: '100%', borderRadius: 5 }} onPress={() => onVerifyEmail()}>
            <Text style={{ fontSize: 20, color: '#fa8526', fontWeight: 'bold' }}>Verify Email</Text>
          </Pressable>
        </View>
      </View>
